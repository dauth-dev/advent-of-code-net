...
namespace AdventOfCode.Day_17
{
	public class Runner : AbstractRunner
	{
        ...
        private void Part2()
		{
            ...
            // 4D-Würfel aufbauen x/y/z/w. Das bitARray ist die "unterste" Scheibe
            var conwayCube = new Dictionary<(int x, int y, int z, int w), bool>();
            for (int i = 0; i < input.Count; i++)
            {
                for (int j = 0; j < input[0].Count; j++)
                {
                    conwayCube[(i, j, 0, 0)] = input[i][j];
                }
            }
            ...
            for (int i = 0; i < 6; i++)
            {
                foreach (var cubeItem in conwayCube)
                {
                    var itemNeighborStates = Neightbors4D(cubeItem.Key, conwayCube);

                    foreach (var neighbor in itemNeighborStates)
                    {
                        // Achtung: die Nachbarn der Nachbarn beeinflussen das Ergebnis
                        // If a cube is inactive but exactly 3 of its neighbors are active, the cube becomes active
                        var neighborsOfNeighbors = Neightbors4D(neighbor.Key, conwayCube).Where(e => conwayCube.ContainsKey(e.Key));
